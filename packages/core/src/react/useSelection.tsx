...
import { apply, between, count, has, NONE, type Ranges, type SelectMode } from "../lib/ranges.ts";
import type { Bucket, Target, TimelineFilter, View } from "../wire/api.ts";
import type { Noun } from "../lib/format.ts";
...
export function SelectionProvider({ children }: { children: ReactNode }) {
  ...
  // Escape used to leave a selection from here. It does not any more: it was a
  // keydown listener and a `document.querySelector`, and neither a keyboard nor
  // a DOM is something this hook can assume. Both moved to
  // web/src/hooks/useSelection, which takes `active` and `exit` off the context
  // and does it in six lines. A phone leaves a selection by pressing Done,
  // which is the same `exit`.
